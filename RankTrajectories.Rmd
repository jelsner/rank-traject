...
JaSun <-   c(8, 8, 8, 8, 8)
Charles <- c(9, 9, 9, 9, 9)
Didi <-    c(10, 10, 10, 10, 10)
Beth <-    c(11, 11, 11, 11, 11)
JimG <-    c(12, 12, 12, 12, 12)
...
ggplot(Long, aes(x = Round, y = Rank, color = Player)) +
  ...
  ggtitle("13th Annual Tally Rally\n Player Rank Trajectory") +
...
ggplot(Long, aes(x = Round, y = Rank, color = Player)) +
  ...
  geom_point(size = 2) +
  xlab("") + ylab("") +
  scale_x_continuous(breaks = 0:4, labels = c("Initial\nSeed", "After\nRound 1", "After\nRound 2", "After\nRound 3", "Final\nRank")) +
  scale_y_reverse(breaks = 1:nP, labels = df$Player) +
  theme_economist() +
  ggtitle("13th Annual Tally Rally\nPlayer Trajectory") +
